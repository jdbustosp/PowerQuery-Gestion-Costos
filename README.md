...
|---|---|---|
| `# CC - Comparativo` | `APROBACIONES_SP` (SharePoint) ↔ `COMPARATIVOS` (manual `Det_CC`) | `FnNormalizeSpaces`: espacios duros (NBSP `#(00A0)`) → normales, espacios dobles internos → uno, trim de extremos. **Aplicada en AMBOS lados** (2026-08) porque el consolidador de SharePoint traía dobles espacios que `Text.Trim` no quita y rompían el match. |
