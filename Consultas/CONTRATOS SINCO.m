let
    ...
    Actual = Table.SelectRows(CONTRATOS, each [#"# OC / Contrato"] <> null and FnDigits([#"# OC / Contrato"]) <> null),
    ...
    FnKeysPrevCC = (cc as text) as table =>
        let
            bin = FnPick(cc, "ESTADO DE CONTRATOS"),
            tbl0 = if bin = null then #table({"Columna1"}, {}) else FnRename(FnTable(bin)),
            withText = Table.AddColumn(tbl0, "FilaTexto", each Text.Combine(List.Transform(List.Select(Record.FieldValues(_), each _ <> null and _ <> ""), each Text.From(_)), " "), type text),
            rows = Table.AddColumn(withText, "Contrato", each if Text.Contains(Text.Upper([FilaTexto]), "CONTRATO NO") then let after = Text.Range([FilaTexto], Text.PositionOf(Text.Upper([FilaTexto]), "CONTRATO NO") + 11) in FnDigits(Text.BeforeDelimiter(Text.TrimStart(after, {".",":"," "}), " ")) else null, type text),
